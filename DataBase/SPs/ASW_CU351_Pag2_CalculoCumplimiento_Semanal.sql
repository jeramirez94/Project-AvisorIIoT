--USE [ASW]
--GO
/****** Object:  StoredProcedure [AswSch].[ASW_CU351_Pag2_CalculoCumplimiento_Semanal]    Script Date: 17/01/2023 10:21:41 a. m. ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROC [AswSch].[ASW_CU351_Pag2_CalculoCumplimiento_Semanal]
 @PnClaUbicacion INT
...
INSERT INTO @SemanaAnio(Anio,Mes,Semana,FechaInicio,FechaFin)
SELECT     Anio
		  ,MesAdm
		  ,SemanaDelAnio
		  ,FechaInicioAdm
...
INSERT INTO @DatosProduccion (ClaUbicacion,ClaveUbicacion,IdComponente,IdCelda,Anio,Mes,Semana,Dia,Hora,ClavosHora)
...
JOIN @SemanaAnio SA
    ON  SA.Anio = YEAR(BitProd.FechaAdm)
	AND SA.Mes = MONTH(BitProd.FechaAdm)
	AND DAY(BitProd.FechaAdm) BETWEEN DAY(SA.FechaInicio) AND DAY(SA.FechaFin)
...
INSERT INTO @CumplimientoA1 (ClaUbicacion,ClaveUbicacion,IdCelda,NomCelda,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,Anio,Mes,Semana,Dia,Hora,CumpleEstandar)
...
BEGIN
	...
	GROUP BY  CA1.Anio,CA1.Mes,CA1.Semana,CA1.ClaUbicacion,CA1.ClaveUbicacion
END
...
BEGIN	
	...
	SELECT  CA1.ClaUbicacion AS ClaUbicacion
		   ...
		   ,AVG(ISNULL(CumpleEstandar,0.0)) as A1
	FROM @CumplimientoA1 CA1
	GROUP BY  CA1.Anio,CA1.Mes,CA1.Semana
	,CA1.ClaUbicacion
	,CA1.ClaveUbicacion
	,CA1.ClaDepartamento
...
****************************/
	...
	INSERT INTO @CumplimientoPrograma (ClaUbicacion,ClaveUbicacion,IdCelda,NomCelda,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,Anio,Mes,Semana,Dia,CumplePrograma)
	...
	JOIN AswSch.AswCatWorkCenterVw Cel
		...
		AND ISNULL(Cel.BajaLogica,0) = 0
	JOIN @SemanaAnio SA
		ON  SA.Anio = YEAR(OP.FechaFinAdm)
		AND SA.Mes = MONTH(OP.FechaFinAdm)
		AND DAY(OP.FechaFinAdm) BETWEEN DAY(SA.FechaInicio) AND DAY(SA.FechaFin)
	JOIN AswSch.TiCatUbicacionVw CU
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCP
		  ...
		  GROUP BY Anio,Mes,Semana,ClaUbicacion) CP
	WHERE Nivel = 1 
	AND   Anio = CP.AnioCP
	AND   Mes =  CP.MesCP
	AND   SemanaAnio =  CP.SemanaCP
	...
	WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
					  WHERE ClaUbicacion = CP.ClaUbicacion 
							AND Anio = CP.Anio 
							AND Mes  = CP.Mes 
							AND SemanaAnio  = CP.Semana
							AND Nivel = 1)
	AND CP.ClaUbicacion IS NOT NULL
	GROUP BY CP.Anio,CP.Mes,CP.Semana,ClaUbicacion,ClaveUbicacion
END	
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCP
				 ...
				 ,AVG(ISNULL(CumplePrograma,0.0)) AS CumplimientoPrograma
		  FROM @CumplimientoPrograma 
		  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaveDepartamento) CP
	WHERE Nivel = 2 
	AND   Anio = CP.AnioCP
	AND   Mes =  CP.MesCP
	AND   SemanaAnio =  CP.SemanaCP
	...
	WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
					  WHERE ClaUbicacion = CP.ClaUbicacion 
							...
							AND Nivel = 2)
	AND CP.ClaUbicacion IS NOT NULL
	GROUP BY CP.Anio,CP.Mes,CP.Semana,ClaUbicacion,ClaveUbicacion,ClaDepartamento,ClaveDepartamento
END
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCP
				 ...
				 ,AVG(ISNULL(CumplePrograma,0.0)) AS CumplimientoPrograma
		  FROM @CumplimientoPrograma 
		  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea) CP
	WHERE Nivel = 3 
	AND   Anio = CP.AnioCP
	AND   Mes =  CP.MesCP
	AND   SemanaAnio =  CP.SemanaCP
	...
	WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
					  WHERE ClaUbicacion = CP.ClaUbicacion 
							...
							AND Nivel = 3)
		 AND CP.ClaUbicacion IS NOT NULL
	GROUP BY CP.Anio,CP.Mes,CP.Semana,ClaUbicacion,ClaveUbicacion,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCP
				 ...
				 ,AVG(ISNULL(CumplePrograma,0.0)) AS CumplimientoPrograma
		  FROM @CumplimientoPrograma 
		  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,IdCelda,NomCelda) CP
	WHERE Nivel = 4
	AND   Anio = CP.AnioCP
	AND   Mes =  CP.MesCP
	AND   SemanaAnio =  CP.SemanaCP
	...
	WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
					  WHERE ClaUbicacion = CP.ClaUbicacion 
							...
							AND Anio = CP.Anio 
							AND Mes  = CP.Mes 
							AND SemanaAnio = Semana
							AND Nivel = 4)
		  AND CP.ClaUbicacion IS NOT NULL
	GROUP BY CP.Anio,CP.Mes,CP.Semana,ClaUbicacion,ClaveUbicacion,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,IdCelda,NomCelda
END

/**************************
*  TIEMPO RESPUESTA ANDON *
***************************/
	INSERT INTO @CumplimientoTiempoAndon (ClaUbicacion,ClaveUbicacion,IdCelda,NomCelda,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,Anio,Mes,Semana,Dia,CumpleTiempo)
	...
	JOIN @SemanaAnio SA
		ON SA.Anio = YEAR(FechaAdmSCADA)
		AND SA.Mes = MONTH(FechaAdmSCADA)
		AND DAY(FechaAdmSCADA) BETWEEN DAY(SA.FechaInicio) AND DAY(SA.FechaFin)
	JOIN AswSch.TiCatUbicacionVw CU
...
BEGIN
	UPDATE @IndicadoresCumplimiento 
		...
		FROM (SELECT  ClaUbicacion as ClaUbicacionCTA
			  ...
			  GROUP BY Anio,Mes,Semana,ClaUbicacion) CTA
		WHERE Nivel = 1 
		AND   Anio = CTA.AnioCTA
		AND   Mes =  CTA.MesCTA
		AND   SemanaAnio =  CTA.SemanaCTA
	...
	SELECT          CTA.ClaUbicacion AS ClaUbicacion
		...
		WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
						  WHERE ClaUbicacion = CTA.ClaUbicacion 
								...
								AND Dia  = CTA.Dia
								AND Nivel = 1)
		GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaveUbicacion
END
...
BEGIN
	UPDATE @IndicadoresCumplimiento 
		...
		FROM (SELECT  ClaUbicacion as ClaUbicacionCTA
			  ...
			  FROM @CumplimientoTiempoAndon CTA 
			  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento) CTA
		WHERE Nivel = 2
		AND   Anio = CTA.AnioCTA
		AND   Mes =  CTA.MesCTA
		AND   SemanaAnio =  CTA.SemanaCTA
	...
	SELECT          CTA.ClaUbicacion AS ClaUbicacion
		...
		WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
						  WHERE ClaUbicacion = CTA.ClaUbicacion 
								...
								AND Anio = CTA.Anio 
								AND Mes  = CTA.Mes 
								AND SemanaAnio  = CTA.Semana
								AND Nivel = 2)
		GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaveUbicacion,ClaDepartamento,ClaveDepartamento
END	
...
BEGIN
	UPDATE @IndicadoresCumplimiento 
		...
		FROM (SELECT  ClaUbicacion as ClaUbicacionCTA
					 ...
					 ,AVG(CumpleTiempo) AS CumplimientoTiempoAndon
			  FROM @CumplimientoTiempoAndon CTA 
			  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaArea) CTA
		WHERE Nivel = 3
		AND   Anio = CTA.AnioCTA
		AND   Mes =  CTA.MesCTA
		AND   SemanaAnio =  CTA.SemanaCTA
	...
	SELECT          CTA.ClaUbicacion AS ClaUbicacion
		...
		WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
						  WHERE ClaUbicacion = CTA.ClaUbicacion 
								...
								AND Anio = CTA.Anio 
								AND Mes  = CTA.Mes 
								AND Semana  = CTA.Semana
								AND Nivel = 3)
		GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaveUbicacion,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea
END
...
BEGIN
	UPDATE @IndicadoresCumplimiento 
		...
		FROM (SELECT  ClaUbicacion as ClaUbicacionCTA
					 ...
					 ,AVG(CumpleTiempo) AS CumplimientoTiempoAndon
			  FROM @CumplimientoTiempoAndon CTA 
			  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaArea,IdCelda) CTA
		WHERE Nivel = 4
		AND   Anio = CTA.AnioCTA
		AND   Mes =  CTA.MesCTA
		AND   SemanaAnio =  CTA.SemanaCTA
	...
	SELECT          CTA.ClaUbicacion AS ClaUbicacion
		...
		WHERE NOT EXISTS (SELECT 1 FROM @IndicadoresCumplimiento 
						  WHERE ClaUbicacion = CTA.ClaUbicacion 
								...
								AND SemanaAnio  = CTA.Semana
								AND Nivel = 4)
		GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaveUbicacion,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,IdCelda,NomCelda
END		
...
**************************/
		...
		INSERT INTO @CumplimientoJustificacionParo (ClaUbicacion,ClaveUbicacion,IdCelda,NomCelda,ClaDepartamento,ClaveDepartamento,ClaArea,ClaveArea,Anio,Mes,Semana,EsJustificado)
		SELECT  CU.ClaUbicacion
			   ,CU.NombreUbicacion
			   ,CEL.IdWorkCenter
			   ,CEL.NomWorkCenter
		...
		JOIN AswSch.AswCatWorkCenterVw CEL
			...
			AND ISNULL(CEL.BajaLogica,0) = 0
		JOIN @SemanaAnio SA
			ON  SA.Anio = YEAR(BitParo.FechaAdmSCADA)
			AND SA.Mes = MONTH(BitParo.FechaAdmSCADA)
			AND DAY(BitParo.FechaAdmSCADA) BETWEEN DAY(SA.FechaInicio) AND DAY(SA.FechaFin)
		JOIN AswSch.TiCatUbicacionVw CU 
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCJP
		  ...
		  GROUP BY Anio,Mes,Semana,ClaUbicacion) CJP
	WHERE Nivel = 1 
	AND   Anio = CJP.AnioCJP
	AND   Mes =  CJP.MesCJP
	AND   SemanaAnio =  CJP.SemanaCJP
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCJP
				 ...
				 ,SUM(EsJustificado)/Count(1) AS ParosJustificados
		  FROM @CumplimientoJustificacionParo CJP 
		  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento) CJP
	WHERE Nivel = 2 
	AND   Anio = CJP.AnioCJP
	AND   Mes =  CJP.MesCJP
	AND   SemanaAnio =  CJP.SemanaCJP
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCJP
				 ...
				 ,SUM(EsJustificado)/Count(1) AS ParosJustificados
		  FROM @CumplimientoJustificacionParo CJP 
		  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaArea) CJP
	WHERE Nivel = 3
	AND   Anio = CJP.AnioCJP
	AND   Mes =  CJP.MesCJP
	AND   SemanaAnio =  CJP.SemanaCJP
...
BEGIN
	...
	FROM (SELECT  ClaUbicacion as ClaUbicacionCJP
				 ...
				 ,SUM(EsJustificado)/Count(1) AS ParosJustificados
		  FROM @CumplimientoJustificacionParo CJP 
		  GROUP BY Anio,Mes,Semana,ClaUbicacion,ClaDepartamento,ClaArea,IdCelda) CJP
	WHERE Nivel = 4
	AND   Anio = CJP.AnioCJP
	AND   Mes =  CJP.MesCJP
	AND   SemanaAnio =  CJP.SemanaCJP
...
--FINAL	
	...
	DELETE FROM [AswSch].[AswEstCumplimientoSemanal]
	WHERE   Anio = @PnAnio
		AND Mes  = @PnMes
	...
	FROM [AswSch].[AswEstCumplimientoSemanal]

	INSERT INTO [AswSch].[AswEstCumplimientoSemanal]
			   (id
			   ,[ClaUbicacion]
			   ,[ClaveUbicacion]
			   ,[Anio]
			   ...
			   ,[NombrePcMod]
			   ,[ClaUsuarioMod])
     SELECT @nId + ROW_NUMBER() OVER (Order by Anio,Mes,Nivel,SemanaAnio)
		   ,ClaUbicacion
           ,ClaveUbicacion
           ,Anio
           ,Mes
